...
__all__ = ["fp8_relu_pg2hp_forward"]
...
@triton.jit
def _fp8_relu_pg2pg_forward_kernel(
    output_ptr,
    ...
    scale_input = tl.load(scale_input_ptr, boundary_check=(0, 1))
    ...
    # Actual Calculation of ReLU
    relu_output = tl.maximum(input, 0)

    # Quantize Scale calculation
    abs_output = tl.abs(relu_output)
    ...
    # Quantize
    relu_output = tl.fdiv(relu_output, scale_output)
    relu_output = relu_output.to(output_ptr.type.element_ty)

    scale_output = scale_output.to(output_scale_ptr.type.element_ty)
    scale_output = tl.reshape(scale_output, (BLOCK_M, BLOCK_SN))
    relu_output = tl.reshape(relu_output, (BLOCK_M, BLOCK_N))
    ...
    tl.store(output_block_ptr, relu_output, boundary_check=(0, 1))
    tl.store(scale_output_ptr, scale_output, boundary_check=(0, 1))


def fp8_relu_pg2pg_forward(x, s_x, QB, transpose_output_2d=False):
    ...
    _fp8_relu_pg2pg_forward_kernel[grid](
        y,
        s_y,
        x,
        s_x,

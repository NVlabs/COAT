...
def _rms_norm_pg2pg_fwd_fused(
    ...
    s_x_stride,  # how much to increase the pointer when moving by 1 row
    y_stride,  # how much to increase the pointer when moving by 1 row
    s_y_stride,  # how much to increase the pointer when moving by 1 row
    ...
    # Scale calculation
    y = tl.reshape(y, (BLOCK_SN2, QB))
    abs_y = tl.abs(y)
    max_val = tl.max(abs_y, axis=1) + SCALE_MIN_THRES
    scale_output = max_val / fp8_max
    scale_output = tl.reshape(scale_output, (BLOCK_SN2, 1))

    # Quantize
    quant_y = tl.fdiv(y, scale_output)
    quant_y = quant_y.to(Y.type.element_ty)

    scale_output = scale_output.to(SY.type.element_ty)
    scale_output = tl.reshape(scale_output, (BLOCK_SN2,))
    quant_y = tl.reshape(quant_y, (N2,))

    # Write output
    tl.store(Y + cols, quant_y, mask=cols < N)
...
def fp8_rmsnorm_pg2pg_forward(x, s_x, w, QB, eps, transpose_output_2d=False):

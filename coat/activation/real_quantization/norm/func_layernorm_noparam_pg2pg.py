...
from ..division._division import _stochastic_rounding
...
@triton.jit
def _layer_norm_pg2pg_fwd_fused(
    ...
    # Scale calculation
    x_hat = tl.reshape(x_hat, (BLOCK_SN2, QB))
    abs_x_hat = tl.abs(x_hat)
    max_val = tl.max(abs_x_hat, axis=1) + SCALE_MIN_THRES
    scale_output = max_val / fp8_max
    scale_output = tl.reshape(scale_output, (BLOCK_SN2, 1))

    # Quantize
    quant_x_hat = tl.fdiv(x_hat, scale_output)
    quant_x_hat = quant_x_hat.to(Y.type.element_ty)

    scale_output = scale_output.to(SY.type.element_ty)
    scale_output = tl.reshape(scale_output, (BLOCK_SN2,))
    quant_x_hat = tl.reshape(quant_x_hat, (N2,))

    # Write output
    tl.store(Y + cols, quant_x_hat, mask=cols < N)
    tl.store(SY + scale_cols, scale_output, mask=scale_cols < SN        )
...
@triton.jit
def _layer_norm_bwd_dx_pg_pg_fused(
...
def fp8_layernorm_noparam_pg2pg_forward(x, s_x, QB, eps, transpose_output_2d=False):
    ...
    s_y = torch.empty(
        (M, SN), dtype=torch.bfloat16, device=x.device
    )
    ...
    return y, s_y, (mean, rstd, num_warps)


def fp8_layernorm_noparam_pg_pg_backward(x, s_x, g, QB, m, v, num_warps, stochastic=False):
    # Change batched 3D input to 2D
    batched = False
    if len(x.shape) == 3:
        assert len(s_x.shape) == 3
